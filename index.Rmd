---
title: Saguaro NP Managers' Report
author: Schoodic Institute at Acadia National Park
output: 
  html_document:
    css: email_alerts/www/css/style.css
github_repo: https://github.com/Kylelima21/Saguaro_managers_report
---
...
inat <- inat_recent("2346", "week")
ebird <- ebird_recent("US-AZ-019")
...
final_data <- combine_citsci_data(
  inat %>%
    select(scientific.name, common.name, iconic.taxon.name, observed.on,
           ...
           user.id, captive.cultivated, url, image.url, license, district = UNIT_NAME, polygonloc),
  ebird %>%
    select(scientific.name, common.name, iconic.taxon.name, count, observed.on,
           place.guess, latitude, longitude, checklist, url, district = UNIT_NAME, polygonloc)) %>%
  filter(polygonloc == "park") %>% 
  mutate(district = ifelse(district == "Saguaro National Park East", "RMD", ifelse(district == "Saguaro National Park West", "TMD", "error"))) %>%
  select(-polygonloc)


## Run the summary function on the final data
# watchlist_species(final_data, "email_alerts/outputs")
sagu_watchlist(final_data, "email_alerts/outputs")
...
records <- bind_rows(
  if(nrow(read.csv("email_alerts/outputs/rmd_obs.csv")) > 0) {
    read.csv("email_alerts/outputs/rmd_obs.csv")
  },

  if(nrow(read.csv("email_alerts/outputs/tmd_obs.csv")) > 0) {
    read.csv("email_alerts/outputs/tmd_obs.csv")
  })

database <- bind_rows(read.csv("email_alerts/outputs/database_records.csv"),
                      records) %>%
  arrange(district, observed.on) %>%
  write.csv(., "email_alerts/outputs/database_records.csv", row.names = F)


## Clean obs for tables
records2 <- records %>% 
...
## Combine for map
rmdmap <- read.csv("email_alerts/outputs/rmd_obs.csv") %>%
  select(scientific.name, common.name, latitude, longitude, url)
tmdmap <- read.csv("email_alerts/outputs/tmd_obs.csv") %>%
  select(scientific.name, common.name, latitude, longitude, url)
mapcomb <- rbind(rmdmap, tmdmap)
...
<div class = "title-box">
  ...
  Saguaro NP Managers' Report
  </div>
  <div class = "h2 header-text">
  A citizen science observation summary
  </div>
  <!-- Intro paragraph -->
  <div class = "headerp"> 
...
<div class = "species-box">
  <div>
  <h3 class = "boxtitles"> RMD Species </h3>
  </div>
  <img src = "email_alerts/www/img/Picture1.jpg" role = "img" aria-label = "A red, black, and pale yellow snake on rocky substrate" class = "rare-img">
  <div class = "sp-table shrink-table">
```{r rmd-table, results = 'asis', echo = FALSE}
if (nrow(records2) > 0) {
  rmdtab <- records2 %>%
    filter(district == "RMD") %>% 
    select(scientific.name, common.name, observed.on, link)
  ...
  DT::datatable(
    ...
    escape = FALSE,
    filter = "top",
    colnames = c("Scientific Name", "Common Name", "Date Observed", "Link"))
} else {
  cat(HTML("<h3>There were no target species reported this week.</h3>"))
}
```
  </div>
</div>
...
<div class = "species-box">
  <div>
  <h3 class = "boxtitles"> TMD Species </h3>
  </div>
  <img src = "email_alerts/www/img/sunrisesaguaro.jpg" role = "img" aria-label = "Sunrise over a saguaro forest" class = "rare-img">
  <div class = "sp-table shrink-table">
```{r tmd-table, results = 'asis', echo = FALSE}
if (nrow(records2) > 0) {
  tmdtab <- records2 %>%
    filter(district == "TMD") %>% 
    select(scientific.name, common.name, observed.on, link)
...
read.csv("email_alerts/outputs/database_records.csv") %>%
  download_this(
    output_name = "full_saguaro_watchlist_database",
...
<div>
  <h2>ABOUT</h2>
  <p><a href = "https://github.com/Kylelima21/Saguaro_managers_report" target="_blank">Source code</a></p>
